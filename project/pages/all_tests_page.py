...
class AllTestsPage(BaseForm):
    ...
    unique_element = Text((By.XPATH, "//a[contains(@href, 'testId')]"),
                          'Unique element for the page identity')
    all_tests_per_page = Text((By.XPATH, "//table[@class='table']"),
                              'Text from tests table')

    def __init__(self):
        self.test_by_id_template = ((By.XPATH, "//a[@href='testInfo?testId=%s']"),
                                    "Test by id button on 'all tests' page")

    def get_all_tests_per_page(self):
        """
